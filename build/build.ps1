...
Invoke-Expression "& `"$CAKE_EXE`" `"$Script`" -target=`"$Target`" -repository=`"$repository`" - -configuration=`"$Configuration`" -apiServerIP=`"$apiServerIP`" -apiServerPort=`"$apiServerPort`" -apiSiteName=`"$apiSiteName`" -apiServerUserName=`"$apiServerUserName`" -apiServerPassword=`"$apiServerPassword`" -verbosity=`"$Verbosity`" $UseMono $UseDryRun $UseExperimental $ScriptArgs"
exit $LASTEXITCODE
